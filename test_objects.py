...
class TestSqliteObjects(unittest.TestCase):
    ...
    def run_dict_tests(self, d):
        ...
        self.assertEqual(
            {("1", 22), ("3", "c"), ("thing", "other")}, set([x for x in d.items()])
        )
    ...
    def test_dict(self):
        ...
        d.write_lines(io)
        values = sorted(
            [line.split("\t") for line in io.getvalue().strip().split("\n")]
        )
    ...
    def test_set(self):
        ...
        s.write_lines(io)
        self.assertEqual(
            {"1", "2", "3", "4", "5"}, set(io.getvalue().strip().split("\n"))
        )
        self.assertEqual(
            ["1", "2", "3", "4", "5"], sorted(io.getvalue().strip().split("\n"))
        )
